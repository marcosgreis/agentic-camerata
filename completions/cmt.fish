...
complete -c cmt -n __fish_use_subcommand -a continue -d 'Continue an existing session'
...
# continue command - complete with session IDs
complete -c cmt -n '__fish_seen_subcommand_from continue' -a 'last' -d 'Continue most recent session'
complete -c cmt -n '__fish_seen_subcommand_from continue' -a '(__cmt_sessions)' -d 'Session ID'
