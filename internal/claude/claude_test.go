...
func TestGetPromptPrefix(t *testing.T) {
	tests := []struct {
		...
		{
			name:       "continue command has no prefix",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := GetPromptPrefix(tt.command)
...
func TestBuildCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "continue ignores task description",
			opts: RunOptions{
				Command:         CommandContinue,
				WorkflowType:    db.WorkflowGeneral,
				ContinueSession: "session-123",
				TaskDescription: "this should be ignored",
			},
			wantArgs:    []string{"-c"},
			notWantArgs: []string{"this should be ignored"},
		},
		{
...
func TestContinueRequiresTmux(t *testing.T) {
	// Ensure we're not in tmux for this test
	original := os.Getenv("TMUX")
	defer os.Setenv("TMUX", original)
	os.Unsetenv("TMUX")
	...
	err = runner.Continue(nil, "test-continue", false)

	if err == nil {
		t.Error("Continue() should return error when not in tmux")
	}
}

func TestContinueSessionNotFound(t *testing.T) {
	// Set TMUX env to pass the tmux check
	original := os.Getenv("TMUX")
	defer os.Setenv("TMUX", original)
	os.Setenv("TMUX", "/tmp/tmux-test,123,0")
	...
	err = runner.Continue(nil, "nonexistent", false)

	if err == nil {
		t.Error("Continue() should return error for nonexistent session")
	}
...
func TestContinueLast(t *testing.T) {
	// Set TMUX env to pass the tmux check
	original := os.Getenv("TMUX")
	defer os.Setenv("TMUX", original)
	os.Setenv("TMUX", "/tmp/tmux-test,123,0")
	...
	// No sessions exist
	err = runner.Continue(nil, "last", false)

	if err == nil {
		t.Error("Continue('last') should return error when no sessions exist")
	}
}
