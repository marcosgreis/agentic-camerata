...
func getVisibleColumns(width int) visibleColumns {
	...
	if used+1+colAgeWidth <= available {
		cols.age = true
		used += 1 + colAgeWidth
	}
...
func (d *Dashboard) formatSessionLine(s *db.Session, cols visibleColumns, selected bool, inHistory bool) string {
	...
	// Truncate prompt to fit column width
	prompt := s.TaskDescription
	if cols.prompt && len(prompt) > cols.promptWidth {
		prompt = prompt[:cols.promptWidth-1] + "…"
	}
	...
	if cols.prompt {
		parts = append(parts, dimStyle.Render(fmt.Sprintf("     %-*s", cols.promptWidth, prompt)))
	}
...
func (d *Dashboard) contentHeight() int {
	available := d.height - 5 // Account for header and help
	if d.showInfo {
		// Give info panel at least 8 lines, list gets the rest
		minInfoHeight := 8
		listHeight := available - minInfoHeight - 2 // -2 for info title
		if listHeight < 5 {
			listHeight = 5
		}
		return listHeight
...
func (d *Dashboard) infoHeight() int {
	available := d.height - 5 // Account for header and help
	height := available - d.contentHeight() - 2 // Remaining space minus title
	if height < 3 {
		height = 3
	}
	return height
